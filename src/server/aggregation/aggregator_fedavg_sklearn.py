...
from server.aggregation.native_weights import num_items, reported_clients, weighted_average
...
def aggregate(
    ...
    logger = FedLogger(id=session_id, loggername="SKLEARN_AGGREGATOR")
    if client_active:
    ...
    if not client_active:
        try:
            ...
            logger.error("sklearn.fedavg.remove_client", str(e))
            raise
    ...
    try:
        ...
        raw_weights = [unwrap_weights(payload) for payload in payloads]
        sample_weights = np.asarray(sample_counts, dtype=float)
        sample_weights = sample_weights / sample_weights.sum()
        coef = weighted_average([weights["coef_"] for weights in raw_weights], sample_weights)
        intercept = weighted_average(
            [weights["intercept_"] for weights in raw_weights], sample_weights
        )
        classes = raw_weights[0].get("classes_")

        aggregator_state.clear()
        return wrap_weights(
            {"coef_": coef, "intercept_": intercept, "classes_": classes},
            backend="sklearn",
        )
    except Exception as e:
        aggregator_state.clear()
        logger.error("sklearn.fedavg.exception", str(e))
        raise
