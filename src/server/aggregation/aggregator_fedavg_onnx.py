...
def aggregate(
    ...
    logger = FedLogger(id=session_id, loggername="ONNX_AGGREGATOR")
    if client_active:
    ...
    if not client_active:
        try:
            ...
            logger.error("onnx.fedavg.remove_client", str(e))
            raise
    ...
    try:
        ...
        aggregator_state.clear()
        return wrap_weights(global_weights, backend="onnx")
    except Exception as e:
        aggregator_state.clear()
        logger.error("onnx.fedavg.exception", str(e))
        raise
